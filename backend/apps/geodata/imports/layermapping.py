...
class LayerMapping:
    ...
    FIELD_TYPES = {
        ...
        models.DecimalField: (OFTReal, OFTInteger),
    ...
    def __init__(self, model, data, mapping, layer=0,
        ...
        self.layer_field_types = OrderedDict(
            [t for t in zip(self.layer.fields, self.layer.field_types)]
        )
    ...
    def check_layer(self):
        ...
        def check_ogr_fld(ogr_map_fld):
            try:
                ...
                raise LayerMapError(
                    'Given mapping OGR field "%s" not found in OGR Layer %s' % (ogr_map_fld, self.layer.name))
            return idx
        ...
        for field_name, ogr_name in self.mapping.items():
            ...
            if isinstance(model_field, GeometryField):
                ...
                if not issubclass(ogr_field, self.FIELD_TYPES[model_field.__class__]):
                    raise LayerMapError('OGR field "%s" (of type %s) cannot be mapped to Django %s for field %s' %
                                        (ogr_field, ogr_field.__name__, fld_name, model_field.name))
                fields_val = model_field
    ...
    def verify_ogr_field(self, ogr_field, model_field):
        ...
        elif isinstance(ogr_field, OFTString) and isinstance(model_field, models.UUIDField):
            # force null or blank UUUID's to be  null
            if ogr_field.value is None or ogr_field.value == '':
                val = None
            else:
                val = ogr_field.value
        elif isinstance(ogr_field, OFTString) and isinstance(model_field, JSONField):
            if ogr_field.value is None or ogr_field.value == '':
                val = None
            else:
                val = json.loads(ogr_field.value)
        else:
            # hack to force values as float for certain gdal drivers
            # that report them as Integer, eg GPKG
            field_type = self.layer_field_types[ogr_field.name]
            if (field_type == OFTReal):
                val = ogr_field.as_double()
            else:
                val = ogr_field.value
        return val
